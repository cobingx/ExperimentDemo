...
csvWrite('auto.csv', ['count','fps', 'rate', 'flag'], 'w')
#计算正确率
rightFlag = 0
righrRate = 0.0
rightLoopTime = 0.0
...
while True:
    ...
    # 添加自适应
    autofactor = autoAdaptive(autofactor, righrRate)
    print('autofactor', autofactor)
    frame_crop, positionAdd = cropImage(frame, posIn[0], posIn[1], posIn[2], posIn[3], autofactor)
    ...
    csvWrite('auto.csv', [frame_counter, fps, righrRate * 100, rightFlag])
